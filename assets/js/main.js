...
var waitTimeBeforeManualConnectionSeconds =
	1000 * waitTimeBeforeManualConnection;
var controller;
...
$(function () {
	...
	$('button.payment_protocol_button').hover(
		function () {
			$(this)
				.find('img')
				.addClass('image_colored')
				.removeClass('image_grayscale');
		},
		function () {
			$(this)
				.find('img')
				.addClass('image_grayscale')
				.removeClass('image_colored');
		}
	);
	...
	$('#wallet-form').on('submit', function (e) {
		...
		fetch(form.action, {
			...
			if (response.ok) {
				...
				$('#floating-submission-box').fadeOut();
			} else {
...
async function startCountDownToManualConnection({ signal }) {
	...
	return new Promise(async (resolve, reject) => {
		let domManualConnectionCount = $(
			'.can_not_connect .seconds_count_to_manual'
		);
		let timeout;
		...
		if (parseInt(domManualConnectionCount.text()) > 0) {
			if (await sleep(1000)) {
				if (parseInt(domManualConnectionCount.text()) > 0) {
					domManualConnectionCount.text(
						parseInt(domManualConnectionCount.text()) - 1
					);
				}
			}
	...
	switch (step) {
		case 1:
			...
			_timeout_ = setTimeout(async () => {
				...
				$('.can_not_connect .seconds_count_to_manual').text(
					waitTimeBeforeManualConnection
				);
				await startCountDownToManualConnection({ signal });
...
function startManualConnection() {
	...
	var launchNewWindow = window.open(
		launchURL,
		'_blank',
		'status=no,titlebar=no,location=no,directories=no,channelmode=no,menubar=no,toolbar=no,scrollbars=no,resizable=no,menubar=0,top=0,left=' +
			window.innerWidth +
